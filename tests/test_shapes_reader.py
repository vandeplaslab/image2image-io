...
def test_read_points_from_df(columns):
    ...
    shapes_geojson, shapes_data = read_shapes_from_df(df)
    assert len(shapes_data) == 1, "Expected to read points from DataFrame."

    df = pd.DataFrame({x_col: [1, 2, 3], y_col: [4, 5, 6], cell_col: ["a", "b", "c"]})
    shapes_geojson, shapes_data = read_shapes_from_df(df)
    assert len(shapes_data) == 3, "Expected to read points from DataFrame."
